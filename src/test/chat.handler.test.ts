...
describe('Chat Handler', () => {
    ...
    describe('createChatSession', () => {
        it('should create a new chat session', async () => {
            ...
            ;(prisma.chatSessions.findFirst as jest.Mock).mockResolvedValueOnce(
                null
            )
            ;(prisma.chatSessions.create as jest.Mock).mockResolvedValueOnce(
                mockSession
            )
        ...
        it('should emit an empty chat history if no message IDs are found', async () => {
            ...
            ;(
                prisma.chatSessions.findUnique as jest.Mock
    ...
    describe('getChatHistory', () => {
        it('should retrieve chat history for a session', async () => {
            ...
            const mockChatSession = {
                id: sessionId,
                msgs: mockMessages.map((m) => m.id),
            }

            ;(prisma.chatSessions.findUnique as jest.Mock).mockResolvedValue(
                mockChatSession
            )
            ;(prisma.msgSessions.findUnique as jest.Mock).mockImplementation(
                (opts) =>
                    Promise.resolve(
                        mockMessages.find((m) => m.id === opts.where.id)
                    )
            )
        ...
        it('should throw an error if chat session is not found', async () => {
            ...
            expect(mockSocket.emit).toHaveBeenCalledWith(
                'error',
                'Cannot get history chat history not found in DB'
            )
        })
