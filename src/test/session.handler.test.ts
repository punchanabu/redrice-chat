...
describe('Session Handler', () => {
    ...
    describe('joinChat', () => {
        it('should join a chat session if session and restaurant user exist', async () => {
            ...
            ;(
                mockChatSessionManager.findChatSession as jest.Mock
            ).mockResolvedValueOnce(session)
            ;(mockPrisma.users.findUnique as jest.Mock).mockResolvedValueOnce(
                restaurantUser
            )
    ...
    describe('sendMessage', () => {
        it('should send a message to the chat session and save it to the database', async () => {
            ...
            mockPrisma.msgSessions.create = jest
                .fn()
                .mockResolvedValueOnce(message)
            mockPrisma.chatSessions.findUnique = jest
                .fn()
                .mockResolvedValueOnce(chatSession)
            mockPrisma.chatSessions.update = jest
                .fn()
        ...
        it('should emit an error if user is not a member of the chat session', async () => {
            ...
            expect(mockSocket.emit).toHaveBeenCalledWith(
                'error',
                'Error: You are not a member of this chat session'
            )
        })
    ...
    describe('getMySession', () => {
        it('should retrieve user sessions and emit them', async () => {
            ...
            ;(
                mockPrisma.chatSessions.findMany as jest.Mock
